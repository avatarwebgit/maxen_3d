...
import EigthStage from './EigthStage';
import NinthStage from './NinthStage';
...
const ContentWrapper = ({ scrollAmount, windowSize }) => {
 ...
 const initialState = () => {
  if (lng === 'en') return { right: '-10%' };
  else return { left: '-20%' };
 };

 const animateNavigation = () => {
  if (lng === 'en') return { right: '1%' };
  else return { left: '1%' };
 };
  ...
  <div className={classes.main}>
   ...
   {visiableStage.fourthStage && (
    <motion.div
     ...
     transition={{ duration: 2 }}
     className={classes.stage_wrapper}>
     <FourthStage scrollAmount={scrollAmount} windowSize={windowSize} />
    </motion.div>
   )}
   ...
   {visiableStage.eigthStage && (
    <motion.div
     initial={initialStageState}
     animate={{ opacity: visiableStage.eigthStage ? 1 : 0 }}
     transition={{ duration: 2 }}
     className={classes.stage_wrapper}>
     <EigthStage scrollAmount={scrollAmount} />
    </motion.div>
   )}

   {visiableStage.ninthStage && (
    <motion.div
     initial={initialStageState}
     animate={{ opacity: visiableStage.ninthStage ? 1 : 0 }}
     transition={{ duration: 2 }}
     className={classes.stage_wrapper}>
     <NinthStage scrollAmount={scrollAmount} />
    </motion.div>
   )}
  </div>
 );
